...
public class DialogAcceptCCPSequenceTextual extends javax.swing.JDialog {
    ...
    public DialogAcceptCCPSequenceTextual(java.awt.Frame parent) {
        super(parent, true);
        initComponents();
        ...
        imgX = new Vector<Integer>();
        imgY = new Vector<Integer>();
        textualPassword = "";
    ...
    class Show3DTimerTask extends TimerTask {
        DialogAcceptCCPSequenceTextual parent;
        int img;

        public Show3DTimerTask(DialogAcceptCCPSequenceTextual parent) {
            this.parent = parent;
            img = 0;
        }
        ...
        public void run() {
            ...
            try {
                Desktop.getDesktop().open(new File(System.getProperty("user.dir") + "\\AcceptPassword\\application.windows32\\AcceptPassword.exe"));
            }catch(Exception e) {
            ...
            while(parent.running) {
                ...
                File f = new File("d:\\temp\\ccp.txt");
                if(f.exists()) {
                    addText("Reading Sequence");
                    String s = "";
                    try {
                        BufferedReader br = new BufferedReader(new FileReader("d:\\temp\\ccp.txt"));
                        String temp = br.readLine();
                        while(temp!=null) {
                            System.out.println("TEMP:" + temp);
                            StringTokenizer st = new StringTokenizer(temp, ",");
                            imgSequence.add(Integer.parseInt(st.nextToken()));
                            imgX.add(Integer.parseInt(st.nextToken()));
                            imgY.add(Integer.parseInt(st.nextToken()));
                            
                            System.out.println("X:" + imgX.get(imgX.size()-1));
                            System.out.println("Y:" + imgY.get(imgY.size()-1));
                            System.out.println("SEQ:" + imgSequence.get(imgSequence.size()-1));
                    ...
                    try {
                        ...
                        System.out.println("PASS: " + textualPassword);
                        br.close();
